...
import { addExifToWebp, DEFAULT_METADATA } from "./rimuru-exif.js";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./rimuru-profile-picture.js";
...
async function extendSocket(sock) {
  ...
  // Tambahkan kredit otomatis ke caption semua media yang dikirim fitur/plugin.
  // Berlaku terpusat supaya tidak perlu mengubah ribuan file plugin satu per satu.
  const featureCreditText = () => {
    const credit = config?.featureCredit;
    if (!credit?.enabled || !credit?.text) return "";
    return String(credit.text).trim();
  };

  const isCaptionMedia = (content) =>
    !!content &&
    typeof content === "object" &&
    (content.image || content.video || content.document);

  const addFeatureCredit = (content) => {
    if (!isCaptionMedia(content)) return content;
    const credit = featureCreditText();
    if (!credit) return content;

    const currentCaption =
      typeof content.caption === "string" ? content.caption.trimEnd() : "";
    if (currentCaption.includes(credit)) return content;

    return {
      ...content,
      caption: currentCaption ? `${currentCaption}\n\n${credit}` : credit,
    };
  };

  sock.sendMessage = async (jid, content, options) => {
    content = addFeatureCredit(content);
    try {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "rimuru-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "rimuru-AI";
    if (source) {
      let data = source;
