...
const fontUrl = getAssetBuffer("rimuru-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("rimuru-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("rimuru-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("rimuru-kertas");
